...
export const generateInsaneHistory = async (db: Database) => {
  return new Promise<{
    ...
    const data = await axios.get(`https://asumatoki.kr/table/insane/data.json`);
    ...
    for (const data of tableData) {
      ...
      const grooveBonus = 50 + parseFloat((1.2 ** numberLevel).toFixed(2));
      ...
      try {
        ...
        if (!row) {
          ...
          const baseScore = parseFloat((FCLevel ** numberLevel).toFixed(2));

          // Full Combo는 BP에 따른 경험치 감소 제외
          const addScore =
            baseScore + 100 + fcBonus + Math.abs(row['rate']) + 0.1;

          const ratingWeight = ((100 - fcRatio) / 4) ** (numberLevel / 10);
          ...
          const baseScore = parseFloat((hardLevel ** numberLevel).toFixed(2));
          ...
          const ratingWeight = ((100 - hardRatio) / 4) ** (numberLevel / 10);
          ...
          const baseScore = parseFloat((grooveLevel ** numberLevel).toFixed(2));
          ...
          const ratingWeight = ((100 - grooveRatio) / 2) ** (numberLevel / 10);
          ...
          const baseScore = parseFloat((easyLevel ** numberLevel).toFixed(2));
          ...
          const ratingWeight = ((100 - easyRatio) / 2) ** (numberLevel / 10);
